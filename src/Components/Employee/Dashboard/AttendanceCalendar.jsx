...
const AttendanceCalendar = () => {
  ...
  const calculateAttendance = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      const currentDateString = moment(currentDate).date(day).format('YYYY-MM-DD');
  ...
  const dayPropGetter = (date) => {
    ...
    let borderStyle = '';
    let tooltipText = `${attendance?.status || 'No Data'} - ${moment(date).format('MMMM Do YYYY')}`;
  ...
  <div className="flex items-center gap-6 flex-wrap">
    ...
    <div className="flex gap-3 flex-wrap">
      ...
      <Legend color="bg-yellow-400" label="Absent" />
      <Legend color="bg-blue-400" label="Holiday" />
      <Legend color="bg-red-500" label="Late" />
    </div>
  </div>
  <input
    type="month"
      ...
      <div className="border border-gray-300 rounded-lg p-6">
        ...
        <div className="mt-6 text-sm text-gray-700">
          <div className="flex justify-between">
            <div>Present: <span className="font-bold">{present} days</span></div>
            <div>Absent: <span className="font-bold">{absent} days</span></div>
            <div>Late: <span className="font-bold">{late} days</span></div>
            <div>Holidays: <span className="font-bold">{holiday} days</span></div>
          </div>
        </div>
      </div>
    </div>
